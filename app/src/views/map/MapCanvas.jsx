...
import { MAINTAIN_COLOR, MAINTAIN_LABEL, VEHICLE_STATE_COLOR, VEHICLE_STATE_LABEL } from '../../styles/theme';
...
export default function MapCanvas({ height = 'calc(100vh - 210px)' }) {
  const { layers, basemap, setBasemap, street, setStreet, fitKey, toggleLayer, setLayerOpacity, setLayerMode } = useMapLayers();
  ...
  return (
    <Box data-map-shell sx={{ position: 'relative', height, minHeight: 420 }}>
      <Paper sx={{ height: '100%', overflow: 'hidden', position: 'relative' }}>
        <MapContainer center={CENTER} zoom={13} style={{ height: '100%', width: '100%' }} preferCanvas zoomControl={false}>
          <TileLayer
            key={basemap}
            url={base.url}
...
function LayerRenderer({ layer }) {
  ...
  if (layer.type === 'line') {
    return layer.data.map((f) => {
      ...
      const color = p.maintainLevel ? (MAINTAIN_COLOR[p.maintainLevel] ?? layer.color) : layer.color;
        ...
        >
          <Popup>
            <Box sx={{ minWidth: 180 }}>
              ...
              {p.routeKm !== undefined && <Typography variant="body2">路線 {Number(p.routeKm).toFixed(2)} km</Typography>}
    ...
    return layer.data.map((v) => {
      ...
      return (
        <CircleMarker
          ...
          center={[v.LAT, v.LNG]}
          radius={v.LIVE ? 10 : 7}
          pathOptions={{ color, fillColor: color, fillOpacity: (v.LIVE ? 0.85 : 0.4) * opacity, weight: v.LIVE ? 3 : 1 }}
        >
          ...
          <Popup>
            <Box sx={{ minWidth: 180 }}>
              ...
              <Stack direction="row" spacing={0.5} sx={{ my: 0.5 }}>
                ...
                {v.SPEED !== null && v.SPEED !== undefined && <Chip size="small" label={`${v.SPEED} km/h`} sx={{ height: 20, fontSize: 11 }} />}
              </Stack>
