 ...
 *    在工地的網路上是很昂貴的浪費。
 */
export default function ImageUploadField({ orderId, canEdit = true, onChanged }) {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await workOrderApi.images(orderId);
  ...
  const pick = async (imgType, file) => {
    ...
    try {
      ...
      await workOrderApi.uploadImages(fd);
      await load();
      onChanged?.();
    } catch (err) {
      setError(err.message);
  ...
  const remove = async (imgType) => {
    setUploading(imgType);
    try {
      await workOrderApi.deleteImage({ ID: orderId, IMG_TYPE: imgType });
      await load();
      onChanged?.();
    } catch (err) {
      setError(err.message);
