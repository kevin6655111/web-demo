...
test.describe('三層組織', () => {
  ...
  test('開不出自己沒有的權限', async ({ request }) => {
    ...
    const bogus = await request.put(`${API}/company/grant`, {
      headers,
      data: { COMPANY_ID: 3, ACTIONS: ['NOT.A.REAL.ACTION'] }
    });
  ...
  test('收回權限會連帶收回下層的那一份', async ({ request }) => {
    ...
    const before = await (await request.get(`${API}/company/3/grant`, { headers })).json();
    ...
    const contractorGrants = await (await request.get(`${API}/company/1/grant`, { headers })).json();
    ...
    const revoke = await (await request.put(`${API}/company/grant`, { headers, data: { COMPANY_ID: 1, ACTIONS: kept } })).json();
    expect(revoke.data.CASCADED_REVOKES).toBeGreaterThan(0);

    // 外包那份跟著失效：留著孤兒授權，等於下層還能做上層已經沒有的事
    const after = await (await request.get(`${API}/company/3/grant`, { headers })).json();
    ...
    // 還原，避免影響其他測試
    await request.put(`${API}/company/grant`, { headers, data: { COMPANY_ID: 1, ACTIONS: [...kept, 'WORK_ORDER.UPDATE'] } });
    await request.put(`${API}/company/grant`, { headers, data: { COMPANY_ID: 3, ACTIONS: subActive } });
  });
});
