...
import { CRACK_TYPE_DEF, DEGREE_DEF, IMAGE_TYPE_DEF, MATERIAL_DEF, REQUIRED_IMAGES } from '@/core/constants/type-def.const';
...
(async () => {
  ...
  for (const c of needOrder) {
    ...
    // PC 是車巡案件轉來的，PD 是 APP 巡查轉來的 —— 兩者都必須帶來源案件
    const type = c.source === 'APP' ? 'PD' : 'PC';
  ...
  logger.log(`   新增案件 ${createdCases} 筆、派工單 ${createdOrders} 張`);
