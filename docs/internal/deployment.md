...
文件裡有完整的欄位結構、業務規則與錯誤條件 —— 那是一份寫得很好的攻擊指南。
所以**預設不開放**，每一份都要金鑰。
...
| 文件     | 路徑                    | 金鑰                | 內容                          |
| -------- | ----------------------- | ------------------- | ----------------------------- |
| 內部     | `/internal-docs`        | `DOCS_INTERNAL_KEY` | 全部 24 個章節                |
| 車機廠商 | `/api-docs/device`      | `DOCS_DEVICE_KEY`   | 只有 `Device` 那兩支          |
| 行動應用 | `/api-docs/app`         | `DOCS_APP_KEY`      | 只有 `Mobile` 那一支          |

金鑰以查詢字串帶入：`/internal-docs?k=<金鑰>`。驗過之後寫一個 httpOnly cookie，
Swagger UI 才抓得到 spec —— 不然畫面出得來、內容是空的。

**一單位一把**是重點：外流時只要換那一把，其他單位不受影響。
共用一把的話，換金鑰等於同時通知所有廠商改設定，於是實務上就永遠不換。

### 內容隔離是兩層

對外文件先依**模組**篩（`modules`），再依**章節 tag** 篩（`tag`）。
兩層的理由是 `@ApiTags` 在類別與方法層是**累加**的：
只靠 tag 篩，一個掛了兩個 tag 的端點會同時出現在兩份文件裡。

所以對接用的端點集中在 `integration/` 這個獨立模組，
而不是散在各領域模組裡加一個 tag。

> ⚠️ 新增端點若沒掛上 `api-docs.const.ts` 列出的任一 tag，**不會出現在任何一份對外文件**。
> 這是刻意的預設：漏列只是少一支（可以補），誤列是把別家的介面攤給對方看。

啟動時會驗證 `INTERNAL_DOC_TAGS` 與程式裡實際用到的 tag 一致，
所以改錯 tag 名稱會在開機時就發現，而不是等廠商回報文件是空的。

### 金鑰沒設定時回 503

未設定或少於 10 字元的金鑰，那一份文件直接回 503 而不是開放。

「忘了設就是全開」這種預設值，出事的時候沒有人會記得自己忘了設。

金鑰比對用 `timingSafeEqual`：一般的字串比較會在第一個不同的字元就回傳，
比對耗時因此洩漏了前綴對了幾個字。這個洩漏量很小，但擋掉它也很便宜。
