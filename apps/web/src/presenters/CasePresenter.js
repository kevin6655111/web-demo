import { CASE_STATUS_COLOR, CASE_STATUS_LABEL, CRACK_LABEL, DEGREE_LABEL, EDITED_LABEL, MAINTENANCE_COLOR, MAINTENANCE_LABEL, MAINTENANCE_TYPE_LABEL, MATERIAL_LABEL, NEED_REPAIR_COLOR, NEED_REPAIR_LABEL, SOURCE_LABEL, WORK_ORDER_LABEL, WORK_ORDER_TYPE_LABEL, WORK_UNIT_LABEL } from '../config/vocabulary';
...
export const CasePresenter = {
  ...
  workUnitLabel: (unit) => WORK_UNIT_LABEL[unit] ?? unit ?? '—',

  /** 巡查單狀態(-1..2)：與案件狀態同一套語意，但是不同的實體 */
  maintenanceLabel: (state) => MAINTENANCE_LABEL[state] ?? state,
  maintenanceColor: (state) => MAINTENANCE_COLOR[state] ?? 'var(--c-neutral)',
  maintenanceTypeLabel: (type) => MAINTENANCE_TYPE_LABEL[type] ?? type,
  ...
  fieldValue(field, value) {
    ...
    if (field === 'crackType' || field === 'dtype') return CRACK_LABEL[value] ?? value;
    if (field === 'workUnit') return WORK_UNIT_LABEL[value] ?? value;
    if (field === 'degree') return DEGREE_LABEL[value] ?? value;
    if (field === 'material') return MATERIAL_LABEL[value] ?? value;
    // type 同時是派工單與巡查單的欄位；兩張表的代碼不重疊，所以依序查兩份對照
    if (field === 'type') return WORK_ORDER_TYPE_LABEL[value] ?? MAINTENANCE_TYPE_LABEL[value] ?? value;
