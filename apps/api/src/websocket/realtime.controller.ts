...
export class RealtimeController {
  ...
  @ApiOperation({
    summary: '線上人員',
    description: [
      '目前有 WebSocket 連線的人，以及各自正在看哪個案件。',
      '狀態有 60 秒 TTL，來源是 Redis。',
      '',
      '所需權限：`CASE.READ`'
    ].join('\n')
  })
  ...
  @ApiOperation({
    ...
    description: [
      '依時間正序回傳最近的討論訊息。',
      'WebSocket 進房時也會自動回補，這支給不開 WebSocket 的情境用。',
      '',
      '所需權限：`CASE.READ`'
    ].join('\n')
  })
  ...
  async handleGetMessages(@Param('ID', ParseIntPipe) id: number, @Query('LIMIT') limit?: string): Promise<HttpResult> {
    ...
    return HttpResponse.successOrWarn({
      data: await this.realtimeService.getRecentMessages(id, take),
      warnMsg: '尚無討論訊息'
    });
  }
  ...
  @ApiOperation({
    ...
    description: [
      '目前這個 api 實例握有的連線數。多實例部署時，各實例的數字要分開看。',
      '',
      '所需權限：`DASHBOARD.READ`'
    ].join('\n')
  })
